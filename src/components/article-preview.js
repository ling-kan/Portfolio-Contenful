  ...
  return (
    <Container>
      <ul className="grid grid-cols-2 gap-3">
        {posts.map((post) => {
          return (
            <Link to={`/blog/${post.slug}`} key={post.slug} className="flex flex-col items-center bg-white rounded-lg border shadow-md md:flex-row md:max-w-xl hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700">
