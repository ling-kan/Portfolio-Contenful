  ...
  return (
    <NavigationWrapper className="my-auto fixed sm:inital z-10 top-0 w-full pin-l pin-t" role="banner">
      <Container as="nav" pageId="navigation">
        <div className={`${mobileNav ? 'bg-primary' : 'bg-primary-background'} sm:bg-transparent rounded-md px-4 py-2 sm:px-0 flex flex-wrap justify-between items-center mx-auto`}>
          <div className="flex items-center text-black">
            <Logo />
          </div>
          <button data-collapse-toggle="mobile-menu" type="button" onClick={() => setMobileNav(!mobileNav)} className="inline-flex items-center p-2 ml-3 text-sm text-grey rounded-lg sm:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 " aria-controls="mobile-menu" aria-expanded="false">
